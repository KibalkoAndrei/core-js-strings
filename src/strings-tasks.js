...
function getStringLength(value) {
  ...
  // Возвращаем длину строки, если это строка
  return value.length;
}
...
function removeFirstOccurrences(str, value) {
  const index = str.indexOf(value); // Находим индекс первого вхождения подстроки
  if (index === -1) {
    return str; // Если подстрока не найдена, возвращаем исходную строку
  }
  return str.slice(0, index) + str.slice(index + value.length); // Убираем первое вхождение
}
...
function startsWith(str, substr) {
  return str.startsWith(substr); // Используем встроенный метод startsWith
}
...
function reverseString(str) {
  // Преобразуем строку в массив символов, затем переворачиваем и объединяем обратно в строку
...
function countVowels(str) {
  // Создаем регулярное выражение для поиска всех гласных
  const vowels = str.match(/[aeiouyAEIOUY]/g);
  // Если гласные найдены, возвращаем их количество, иначе возвращаем 0
...
function getCardId(value) {
  ...
  // Find and return the index of the given card
